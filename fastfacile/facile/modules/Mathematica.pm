...
#
# Synopsys: Export model in Mathematica format.
##############################################################################
...
#---------------------------------------------------#
# Prints the input file for Mathematica.  Requires  #
# that term lists be previously generated.          #
#---------------------------------------------------#
sub export_mathematica_file {
    my $self = shift;
	...
	foreach my $reaction_ref (@create_reactions) {
	    ...
	    # strip out multiplication '*' and replace with ' '
	    $velocity =~ s/\*/ /g;
	    $ode_rhs .= "+ $velocity ";
	}
